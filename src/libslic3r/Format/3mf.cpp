...
// 0 : 3MF files saved by older PrusaSlicer. No version definition in them.
    ...
    bool _3MF_Importer::_handle_end_metadata()
    {
        ...
        if (m_curr_metadata_name == SLIC3RPE_FDM_SUPPORTS_PAINTING_VERSION) {
            ...
            check_painting_version(m_fdm_supports_painting_version, FDM_SUPPORTS_PAINTING_VERSION,
                (boost::format(_(L("The selected 3MF contains FDM supports painted object using a newer version of %1% and is not compatible."))) % std::string(SLIC3R_APP_NAME)).str());
        }

        if (m_curr_metadata_name == SLIC3RPE_SEAM_PAINTING_VERSION) {
            m_seam_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
            check_painting_version(m_seam_painting_version, SEAM_PAINTING_VERSION,
                (boost::format(_(L("The selected 3MF contains seam painted object using a newer version of %1% and is not compatible."))) % std::string(SLIC3R_APP_NAME)).str());
        }

        if (m_curr_metadata_name == SLIC3RPE_MM_PAINTING_VERSION) {
            m_mm_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
            check_painting_version(m_mm_painting_version, MM_PAINTING_VERSION,
                (boost::format(_(L("The selected 3MF contains multi-material painted object using a newer version of %1% and is not compatible."))) % std::string(SLIC3R_APP_NAME)).str());
        }
